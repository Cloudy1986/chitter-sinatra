...
class Comment
  ...
  def self.create(text:, peep_id:, user_id:)
    result = database_connection.exec_params('INSERT INTO comments (text, peep_id, user_id) VALUES ($1, $2, $3) RETURNING id, text, created_at, peep_id, user_id;', [text, peep_id, user_id])
    Comment.new(
      id: result[0]['id'],
      text: result[0]['text'],
  ...
  def self.find(peep_id:)
    result = database_connection.exec_params('SELECT * FROM comments WHERE peep_id = $1;', [peep_id])
