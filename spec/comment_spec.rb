...
describe Comment do
  ...
  describe '.find' do
    it 'returns the comments for a peep' do
      user = User.create(username: 'Bob', email: 'bob@example.com', password: 'password1234')
      peep = Peep.create(message: 'Test peep 2', user_id: user.id)
      comment_one = Comment.create(text: 'This is a comment', peep_id: peep.id, user_id: user.id)
      comment_two = Comment.create(text: 'This is another comment', peep_id: peep.id, user_id: user.id)

      comments = Comment.find(peep_id: peep.id)

      expect(comments.length).to eq 2
      expect(comments[0]).to be_a Comment
      expect(comments[0].text).to eq comment_one.text
      expect(comments[1].id).to eq comment_two.id
      expect(comments[0].peep_id).to eq comment_one.peep_id
      expect(comments[1].user_id).to eq comment_two.user_id
    end
  end
